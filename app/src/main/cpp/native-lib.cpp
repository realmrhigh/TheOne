...
public:
    oboe::DataCallbackResult onAudioReady(
        ...
        // Cast audioData to float* since we're using AudioFormat::Float
        float *outputBuffer = static_cast<float*>(audioData);

        // Clear the buffer to silence first. Output is stereo.
        ...
        for (auto soundIt = gActiveSounds.begin(); soundIt != gActiveSounds.end(); /* no increment here */) {
            if (!soundIt->isActive.load()) { // Check atomic bool
                // Optional: If relying on this loop for cleanup, can increment and continue
                // but usually cleanup is done after processing all active sounds.
                // For now, let's assume cleanup happens after this loop.
                // If a sound becomes inactive mid-callback, it will just stop contributing.
                ++soundIt;
                continue;
            }

            theone::audio::PlayingSound &sound = *soundIt; // Get a reference
            const theone::audio::LoadedSample* loadedSample = sound.loadedSamplePtr;

            if (!loadedSample || loadedSample->audioData.empty()) {
                sound.isActive.store(false); // Mark inactive if no data
                ++soundIt;
                continue;
            }

            int channels = oboeStream->getChannelCount(); // Should be 2 for stereo
            int sampleChannels = loadedSample->format.channels;

            for (int i = 0; i < numFrames; ++i) {
                if (!sound.isActive.load()) { // Re-check, might have been marked by other logic
                    break;
                }

                size_t currentReadFrame = sound.currentFrame;
                if (currentReadFrame >= loadedSample->frameCount) {
                    sound.isActive.store(false); // End of sample
                    break; // Stop processing this sound for this callback
                }
                ...
                if (sampleChannels == 1) { // Mono sample
                ...
                } else { // Stereo sample (assuming loadedSample->format.channels == 2)
                    // If sample has more channels than 2, this simple logic only takes the first two.
                    ...
                    // Apply per-voice pan/gain. For direct stereo, gainLeft/Right act as channel volumes.
                    leftSampleValue = L * sound.gainLeft;
                    rightSampleValue = R * sound.gainRight;
                }

                // Mix into output buffer (assuming output is stereo)
                if (channels == 2) {
                    outputBuffer[i * 2]     += leftSampleValue;
                    outputBuffer[i * 2 + 1] += rightSampleValue;
                } else if (channels == 1) { // Mono output (less common for music apps)
                    outputBuffer[i] += (leftSampleValue + rightSampleValue) * 0.5f; // Mix L/R to mono
                }

                sound.currentFrame++; // Advance frame for this specific sound
            }
             ++soundIt; // Move to next sound in gActiveSounds
        }

        // Clean up inactive sounds from gActiveSounds
