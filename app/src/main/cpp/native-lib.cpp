...
Java_com_example_theone_audio_AudioEngine_native_1playPadSample(
        ...
        // LFO 1 (New)
        jboolean jHasLfo1,
        jboolean jLfo1IsEnabled,
        jint jLfo1WaveformOrdinal,
        jfloat jLfo1RateHz,
        jboolean jLfo1SyncToTempo,
        jint jLfo1TempoDivisionOrdinal,
        jfloat jLfo1Depth,
        jint jLfo1PrimaryDestinationOrdinal
) {
    ...
    std::shared_ptr<theone::audio::PadSettingsCpp> padSettingsPtr;
    {
        ...
        if (it != gPadSettingsMap.end()) {
            ...
            // LFO 1 (from JNI params, if jHasLfo1 is true)
            if (jHasLfo1 == JNI_TRUE && jLfo1IsEnabled == JNI_TRUE) {
                theone::audio::LfoSettingsCpp lfo1FromParams;
                lfo1FromParams.id = "lfo1_fallback"; // Assign a default ID
                lfo1FromParams.isEnabled = true;
                lfo1FromParams.waveform = static_cast<theone::audio::LfoWaveformCpp>(jLfo1WaveformOrdinal);
                lfo1FromParams.rateHz = jLfo1RateHz;
                lfo1FromParams.syncToTempo = jLfo1SyncToTempo;
                lfo1FromParams.tempoDivision = static_cast<theone::audio::TimeDivisionCpp>(jLfo1TempoDivisionOrdinal);
                lfo1FromParams.depth = jLfo1Depth;
                lfo1FromParams.primaryDestination = static_cast<theone::audio::LfoDestinationCpp>(jLfo1PrimaryDestinationOrdinal);

                float tempo = 120.0f; // Default tempo for fallback
                if (gCurrentSequence) {
                ...
                auto lfoGen = std::make_unique<theone::audio::LfoGenerator>();
                lfoGen->configure(lfo1FromParams, sr, tempo);
                lfoGen->retrigger();
                soundInFallback.lfoGens.push_back(std::move(lfoGen));
                __android_log_print(ANDROID_LOG_INFO, APP_NAME, "Fallback sound: Configured LFO1: id=%s, wave=%d, rate=%.2f, depth=%.2f, dest=%d, sync=%d, div=%d",
                                    lfo1FromParams.id.c_str(), jLfo1WaveformOrdinal, jLfo1RateHz, jLfo1Depth, jLfo1PrimaryDestinationOrdinal, jLfo1SyncToTempo, jLfo1TempoDivisionOrdinal);
            } else if (jHasLfo1 == JNI_TRUE && jLfo1IsEnabled == JNI_FALSE) {
                 __android_log_print(ANDROID_LOG_INFO, APP_NAME, "Fallback sound: LFO1 is present but not enabled via jLfo1IsEnabled.");
            }
    ...
    // Log JNI LFO parameters even if using PadSettings (for debugging/verification)
    if (jHasLfo1 == JNI_TRUE) {
        __android_log_print(ANDROID_LOG_DEBUG, APP_NAME, "PadSettings path: JNI LFO1 params: isEnabled=%d, wave=%d, rate=%.2f, depth=%.2f, dest=%d, sync=%d, div=%d",
                            jLfo1IsEnabled, jLfo1WaveformOrdinal, jLfo1RateHz, jLfo1Depth, jLfo1PrimaryDestinationOrdinal, jLfo1SyncToTempo, jLfo1TempoDivisionOrdinal);
    }
...
theone::audio::EnvelopeSettingsCpp ConvertKotlinEnvelopeSettings(JNIEnv* env, jobject kotlinEnvelopeSettings) {
    ...
    jfieldID typeFid = env->GetFieldID(envSettingsClass, "type", "Lcom/example/theone/model/ModelEnvelopeTypeInternal;");
    ...
    // And Kotlin enum com.example.theone.model.ModelEnvelopeTypeInternal { AD, AHDS, ADSR };
...
theone::audio::LfoSettingsCpp ConvertKotlinLfoSettings(JNIEnv* env, jobject kotlinLfoSettings) {
    ...
    jfieldID waveformFid = env->GetFieldID(lfoSettingsClass, "waveform", "Lcom/example/theone/model/LfoWaveform;");
    ...
    jfieldID tempoDivisionFid = env->GetFieldID(lfoSettingsClass, "tempoDivision", "Lcom/example/theone/model/TimeDivision;");
    jfieldID depthFid = env->GetFieldID(lfoSettingsClass, "depth", "F");
    jfieldID primaryDestinationFid = env->GetFieldID(lfoSettingsClass, "primaryDestination", "Lcom/example/theone/model/LfoDestination;");
...
Java_com_example_theone_audio_AudioEngine_native_1updatePadSettings(
    ...
    jfieldID layerTriggerRuleFid = env->GetFieldID(padSettingsClass, "layerTriggerRule", "Lcom/example/theone/model/LayerTriggerRule;");
    ...
    jfieldID playbackModeFid = env->GetFieldID(padSettingsClass, "playbackMode", "Lcom/example/theone/model/PlaybackMode;");
    ...
    jfieldID ampEnvelopeFid = env->GetFieldID(padSettingsClass, "ampEnvelope", "Lcom/example/theone/model/EnvelopeSettings;");
    ...
    jfieldID pitchEnvelopeFid = env->GetFieldID(padSettingsClass, "pitchEnvelope", "Lcom/example/theone/model/EnvelopeSettings;");
    ...
    jfieldID filterEnvelopeFid = env->GetFieldID(padSettingsClass, "filterEnvelope", "Lcom/example/theone/model/EnvelopeSettings;");
