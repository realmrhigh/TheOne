...
class AudioEngine(private val context: Context) : AudioEngineControl {
    ...
    private external fun native_playPadSample(
        ...
        // LFO 1 (New)
        hasLfo1: Boolean,
        lfo1IsEnabled: Boolean,
        lfo1WaveformOrdinal: Int,
        lfo1RateHz: Float,
        lfo1SyncToTempo: Boolean,
        lfo1TempoDivisionOrdinal: Int,
        lfo1Depth: Float,
        lfo1PrimaryDestinationOrdinal: Int
    ): Boolean

    // JNI declaration for sample playback (Slice related)
    private external fun native_playSampleSlice(
        sampleId: String, noteInstanceId: String, volume: Float, pan: Float, sampleRate: Int,
        trimStartMs: Long, trimEndMs: Long,
        loopStartMs: Long, loopEndMs: Long, // Changed Boolean to Long here
        isLooping: Boolean
    ): Boolean
    ...
    override suspend fun playPadSample(
        ...
        Log.d("AudioEngine", "playPadSample called for pad $padId with full envelopes & LFO1")

        // Default values for nullable envelopes (already present from previous step)
        val defaultTypeOrdinal = 0 // Assuming AD is ordinal 0 and a safe default
        val defaultEnvAttack = 0f
        val defaultEnvHold = 0f
        val defaultEnvDecay = 0f
        val defaultEnvSustain = 1f
        val defaultEnvRelease = 0f
        val defaultEnvVelToAttack = 0f
        val defaultEnvVelToLevel = 0f

        val lfo1 = lfos.firstOrNull() // Get the first LFO if available

        // Default values for LFO parameters
        val defaultLfoWaveformOrdinal = 0 // e.g., SINE
        val defaultLfoRateHz = 1.0f
        val defaultLfoTempoDivisionOrdinal = 0 // e.g., Quarter
        val defaultLfoDepth = 0.0f
        val defaultLfoPrimaryDestinationOrdinal = 0 // e.g., NONE

        return native_playPadSample(
            noteInstanceId, trackId, padId, sampleId, sliceId,
            ...
            // LFO 1 Parameters (New)
            lfo1 != null,
            lfo1?.isEnabled ?: false,
            lfo1?.waveform?.ordinal ?: defaultLfoWaveformOrdinal,
            lfo1?.rateHz ?: defaultLfoRateHz,
            lfo1?.syncToTempo ?: false,
            lfo1?.tempoDivision?.ordinal ?: defaultLfoTempoDivisionOrdinal,
            lfo1?.depth ?: defaultLfoDepth,
            lfo1?.primaryDestination?.ordinal ?: defaultLfoPrimaryDestinationOrdinal
        )
    }
    ...
    override suspend fun playSampleSlice(
        ...
        return native_playSampleSlice(
            ...
            loopStartMs ?: 0L,
            loopEndMs ?: 0L, // This is now correctly passing a Long to a Long JNI param
            isLooping
        )
    }
